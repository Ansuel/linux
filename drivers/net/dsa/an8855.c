...
static void an8855_port_fast_age(struct dsa_switch *ds, int port)
{
	...
	return an8855_fdb_cmd(priv, AN8855_ATC_MAT(AND8855_FDB_MAT_MAC_TYPE_PORT) |
...
static int an8855_setup(struct dsa_switch *ds)
{
	...
	dsa_switch_for_each_port(dp, ds) {
		...
		ret = regmap_clear_bits(priv->regmap, AN8855_PSC_P(dp->index), AN8855_SA_DIS);
		if (ret)
			return ret;
	...
	ret = regmap_set_bits(priv->regmap, AN8855_PSC_P(AN8855_CPU_PORT), AN8855_SA_DIS);
	if (ret)
		return ret;
