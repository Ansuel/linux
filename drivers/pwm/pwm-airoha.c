 ...
 *  - Only 8 concurrent waveform generator for 8 combination of duty_cycle
 *    and period are possible shared between 16 GPIO pin and 17 SIPO GPIO
 *    pin.
...
static u32 airoha_pwm_rmw(void __iomem *addr, u32 mask, u32 val)
{
	val |= (readl(addr) & ~mask);
	writel(val, addr);
...
#define airoha_pwm_set_bits(void __iomem *addr, u32 val)				\
	airoha_pwm_rmw(addr, val, val)

#define airoha_pwm_clear_bit(void __iomem *addr, u32 mask)				\
	airoha_pwm_rmw(addr, mask, 0)
...
static void airoha_pwm_release_bucket_config(struct airoha_pwm *pc, unsigned int hwpwm)
{
	int i;
...
static int airoha_pwm_sipo_init(struct airoha_pwm *pc)
{
	...
	airoha_pwm_sgpio_clear(pc, REG_SGPIO_LED_DATA, SGPIO_LED_DATA);
	if (readl_poll_timeout(pc->base + REG_SGPIO_LED_DATA, val,
			       !(val & SGPIO_LED_SHIFT_FLAG), 10,
			       200 * USEC_PER_MSEC))
		return -ETIMEDOUT;
					  ...
					  u64 duty_ns, u64 period_ns)
{
	...
	duty = clamp_val(div64_u64(clamp_val(DUTY_FULL * duty_ns), period_ns),
			 0, DUTY_FULL);
			     ...
			     u64 duty_ns, u64 period_ns)
{
	...
	index = airoha_pwm_consume_generator(pc, duty_ns, period_ns, pwm->hwpwm);
	if (index < 0)
		return -EBUSY;
	...
	if (index >= 0) {
		airoha_pwm_get_bucket_config(pc, index, duty_ns, period_ns);
